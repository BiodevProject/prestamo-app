...
public interface CreditoCuotaRepository extends JpaRepository<CreditoCuotaEntity, Long> {
    @Query("SELECT cc FROM CreditoCuotaEntity cc WHERE LOWER(cc.estado) = 'pendiente'")
    ...
    @Query("SELECT cc FROM CreditoCuotaEntity cc WHERE LOWER(cc.estado) = 'pagado'")
    ...
    @Query("SELECT cc FROM CreditoCuotaEntity cc WHERE cc.credito.id = :creditoId AND LOWER(cc.estado) = 'avencer'")
    List<CreditoCuotaEntity> findAVencerByCreditoId(@Param("creditoId") Long creditoId);
